...
setTimeout(() => console.log("0 sec timer"), 0); //calback queue
...
Promise.race([
  getJSON(`https://restcountries.com/v2/name/republic of india?fullText=true`),
  timeout(0.1),
])
...
const loadAll = async function (imagArr) {
  try {
    const imgs = imagArr.map(async (img) => await createImage(img));
    console.log(imgs);
